...
import com.example.thecatapp.ui.components.Register.RedirectToMain
import com.example.thecatapp.ui.components.Register.RegisterFooter
import com.example.thecatapp.ui.navigation.RegisterNavigator
...
fun RegisterDisplayer(
    ...
    AppScaffold(
        ...
        Column(
            ...
            if(uiState.value is RegisterUiState.EmailHasBeenSent)
                RedirectToMain(
                    onRedirectClick = { navigator.goBack(activity) }
                )
            else{
                LogInHeader(
